...
This queries the Control API on `127.0.0.1:9093` and shows every run (including status, elapsed time, etc.). Use it to confirm whether a run is still active before shutting things down. If you need to stop a specific run gracefully:
...
3. `flwr list . remote-federation` — should still connect (even if no runs are active), confirming the CLI can reach the Control API once you start it again.
